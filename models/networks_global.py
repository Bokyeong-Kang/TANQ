...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=RN_B, use_dropout=use_dropout, use_bias=True)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            seg_decoder += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                               kernel_size=3, stride=2,
                                               padding=1, output_padding=1,
                                               bias=True),
                            norm_layer(int(ngf * mult / 2)),
                            nn.ReLU(True)]
        seg_decoder += [nn.ReflectionPad2d(3)]
        seg_decoder += [nn.Conv2d(ngf, 3, kernel_size=7, padding=0)]
        self.seg_decoder = nn.Sequential(*seg_decoder)


        self.model = nn.ModuleList(model)

    def forward(self, input, mask=None, layers=[], encode_only=False):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if (layer_name.find('RAIN') == -1) and (layer_name.find('ResnetBlock') == -1) and (layer_name.find('RN_B') == -1):
                    self.feat = layer(self.feat)
                elif (layer_name.find('RAIN') == -1):
                    self.feat = layer(self.feat, mask_cochlea)
                else:
                    self.feat = layer(self.feat, mask)
                if layer_id in layers:
                    feats.append(self.feat)
                else:
                    pass
            ...
            return self.feat, feats  # return both output and intermediate features
        else:
            """Standard forward"""
            self.x = input

            mask_cochlea = torch.where(mask<2.0,torch.tensor(0, dtype=mask.dtype, device=mask.device),mask)
            for std_layer in self.model:
                layer_name = std_layer.__class__.__name__
                if (layer_name.find('RAIN') == -1) and (layer_name.find('ResnetBlock') == -1) and (layer_name.find('RN_B') == -1):
                    self.x = std_layer(self.x)
                elif (layer_name.find('RAIN') == -1):
                    if layer_name.find('ResnetBlock') != -1:
                        self.resout = std_layer(self.x, mask_cochlea)
                        self.seg = self.seg_decoder(self.resout)
                        self.x = self.resout
                    else:
                        self.x = std_layer(self.x, mask_cochlea) # RN, Resblocks
                else:
                    self.x = std_layer(self.x, mask) # RAIN
            fake = self.x

            return fake, self.seg
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), RN_B(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), RN_B(dim)]
    ...
    def forward(self, x, mask):
        ...
        for conv_layer in self.conv_block:
            layer_name = conv_layer.__class__.__name__
            if layer_name.find('RN_B') == -1:
                self.x = conv_layer(self.x)
            else:
                self.x = conv_layer(self.x, mask_cochlea)
        out = x + self.x
        return out
